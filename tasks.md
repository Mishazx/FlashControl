...
локальная очередь
   ↓
проверка доступности Main
   ↓
┌──────────────┬──────────────┐
│ Main доступен│ недоступен   │
└──────┬───────┴──────┬───────┘
       │              │
       ▼              ▼
      Main       подходящий Proxy
                       │
                       ▼
                      Main
```
...
Проверять реальный application endpoint:
